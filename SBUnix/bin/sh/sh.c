...
void cat(char *name)
{
    ...
    temp[len] = '\0';
    //print("\n filename %s", temp);
    uint64_t fd = open(temp + 1);
    char buff[256];
    int j = 0;
    if (fd != 0)
    {
        int bytes= readf(fd, 256, (uint64_t)buff);
        printf("%d bytes read.\n", bytes);
        while(bytes-- != 0)
            printf("%c", buff[j++]);
    }
                ...
                if(is_file_exist)
                {
                    ...
                    for(i = 0; i<512; i++)
                    {
                        ...
                        else if(buf[i] == '\n')
                        {
                            ...
                            if(first == 0)
                            {
                                if(strcmp1(trimwhitespace(temp), "#!/bin/sh") != 0)
                                {
...
void execute(char * input)
{
        ...
        { 
            ...
            //print("You entered %s \n", directory);
            if(*directory == '\0' || (strcmp1(directory, "/") == 0) )
            {
                pwd[0] =  '~';
                pwd[1] =  '\0';
            }
        ...
        {
           // printf("\nSupported commands are : cd, cd .., ls, cat, shutdown, echo, sleep, ps, sh, man");
          //  shutdown();
        }
...
void *
memcpy1(void *dst, const void *src, size_t n) {
